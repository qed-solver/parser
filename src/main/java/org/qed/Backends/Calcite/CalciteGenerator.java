...
//    }
    ...
    public Env transformPred(Env env, RexRN.Pred pred) {
        if (env.rulename.equals("JoinCommute")) {
            ...
            return env.focus("org.qed.Backends.Calcite.CalciteUtilities.mapFilterToProjectedColumns(call)");
        } 
            ...
            return env.focus("org.qed.Backends.Calcite.CalciteUtilities.mapFilterToAggregatedColumns(call)");
        }
        else if (env.rulename.equals("FilterAggregateTranspose")) {
            return env.focus("org.qed.Backends.Calcite.CalciteUtilities.pushFilterPastAggregate(call)");
        }
        else {
    ...
    public Env transformJoin(Env env, RelRN.Join join) {
        if (env.rulename.equals("JoinConditionPush")) {
            ...
            var leftCondDecl = envWithBuilder.declare(
                "org.qed.Backends.Calcite.CalciteUtilities.ConditionDecomposer.extractLeftOnlyConditions(" +
            ...
            var rightCondDecl = envWithLeftCond.declare(
                "org.qed.Backends.Calcite.CalciteUtilities.ConditionDecomposer.extractRightOnlyConditions(" +
                "((LogicalJoin) call.rel(0)).getCondition(), " +
            ...
            var joinCondDecl = envWithRightCond.declare(
                "org.qed.Backends.Calcite.CalciteUtilities.ConditionDecomposer.extractJoinConditions(" +
                "((LogicalJoin) call.rel(0)).getCondition(), " +
                "call.rel(1).getRowType().getFieldCount(), " +
                "call.rel(1).getRowType().getFieldCount() + call.rel(2).getRowType().getFieldCount(), call)"
            );
    ...
    public Env transformProject(Env env, RelRN.Project project) {
        if (env.rulename.equals("ProjectMerge")) {
            return env.focus("org.qed.Backends.Calcite.CalciteUtilities.mergeProjections(call)");
        }
    ...
    public Env transformAggregate(Env env, RelRN.Aggregate aggregate) {
        if (env.rulename.equals("AggregateProjectMerge")) {
            return env.focus("org.qed.Backends.Calcite.CalciteUtilities.createMergedAggregateProject(call)");
        }
        else if (env.rulename.equals("AggregateExtractProject")) {
            return env.focus("org.qed.Backends.Calcite.CalciteUtilities.extractProjectForAggregate(call)");
        }

...
public class FilterAggregateTranspose extends RelRule<FilterAggregateTranspose.Config> {
	...
	public void onMatch(RelOptRuleCall call) {
		...
		var var_5 = var_3.push(call.rel(2)).filter(org.qed.Backends.Calcite.CalciteUtilities.pushFilterPastAggregate(call)).groupKey(var_4);
		var var_6 = ((LogicalAggregate) call.rel(1)).getAggCallList();
		call.transformTo(var_3.push(call.rel(2)).filter(org.qed.Backends.Calcite.CalciteUtilities.pushFilterPastAggregate(call)).aggregate(var_5, var_6).build());
	}

...
public class AggregateJoinRemove extends RelRule<AggregateJoinRemove.Config> {
	...
	public void onMatch(RelOptRuleCall call) {
		...
		var var_12 = ((LogicalAggregate) call.rel(0)).getAggCallList();
		call.transformTo(var_10.push(call.rel(3)).push(call.rel(4)).join(JoinRelType.INNER, var_10.literal(true)).aggregate(var_10.groupKey(var_11), var_12).build());
	}

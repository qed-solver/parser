...
public class CockroachGenerator implements CodeGenerator<CockroachGenerator.Env> {
    ...
    public Env transformCustom(Env env, RelRN custom) {
        ...
        if (custom instanceof org.qed.RRuleInstances.UnionPullUpConstants.SourceTable sourceTable) {
            String varName = env.bindings().getOrDefault("leftInput", env.bindings().getOrDefault("rightInput", env.bindings().getOrDefault("left", env.bindings().getOrDefault("right", "input_0"))));
            return env.setPattern("$" + varName).focus("$" + varName);
        }
    ...
    public String translate(String name, Env onMatch, Env transform) {
        ...
        if (match.contains("HasZeroRows")) {
            match = normalizeVars(match, new String[]{"projections", "passthrough", "filters"});
        }
        ...
        } else if (out.equals("$input") && numbered != null) {
            out = "$" + numbered;
        }
        ...
        if (match.contains("HasZeroRows")) {
            out = normalizeVars(out, new String[]{"projections", "passthrough", "filters"});
        }

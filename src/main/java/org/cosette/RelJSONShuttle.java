    ...
    public static void dumpToJSON(List<RelNode> relNodes, File file) throws IOException {
        ...
        var schemas = array(tables.map(table -> object(Map.of("name", new TextNode(table.getName()), "fields",
                ...
                array(table.getConstraints()
                        .map(check -> new RexJSONVisitor(shuttle.env.advanced(table.getColumnNames().size())).visit(
    ...
    public JsonNode visit(RelNode rel) {
        return switch (rel) {
            ...
            case LogicalValues values -> {
                ...
                var records =
                        array(Seq.from(values.getTuples()).map(tuple -> array(Seq.from(tuple).map(visitor::visit))));
            ...
            case LogicalFilter filter -> {
                ...
                yield object(Map.of("filter", object(Map.of("condition", visitor.visit(filter.getCondition()), "source",
            ...
            case LogicalProject project -> {
                ...
                yield object(Map.of("project", object(Map.of("target", targets, "source", visit(project.getInput())))));
            }
            case LogicalJoin join -> {
                ...
                yield object(Map.of("join",
                        object(Map.of("kind", new TextNode(join.getJoinType().toString()), "condition",
                                visitor.visit(join.getCondition()), "left", visit(left), "right", visit(right)))));
            }
            case LogicalCorrelate correlate -> {
                ...
                yield object(Map.of("correlate",
                        array(Seq.of(visit(correlate.getLeft()), rightShuttle.visit(correlate.getRight())))));
            }
            case LogicalAggregate aggregate -> {
                ...
                var keys = object(Map.of("project",
                        object(Map.of("target", keyCols, "source", visit(aggregate.getInput())))));
                ...
                var aggregated = object(Map.of("aggregate", object(Map.of("function", aggs, "source",
                        object(Map.of("filter", object(Map.of("condition", condition, "source",
                                new RelJSONShuttle(env.lifted(groupCount)).visit(aggregate.getInput())))))))));
            ...
            case LogicalSort sort -> {
                ...
                var args = object(Map.of("collation", collations, "source", visit(sort.getInput())));
            ...
            return switch (rex) {
                ...
                case RexSubQuery subQuery ->
                        object(Map.of("operator", new TextNode(subQuery.getOperator().toString()), "operand",
                                array(Seq.from(subQuery.getOperands()).map(this::visit)), "query",
                                new RelJSONShuttle(env.advanced(0)).visit(subQuery.rel), "type",
                ...
                case RexCall call -> object(Map.of("operator", new TextNode(call.getOperator().toString()), "operand",
                        array(Seq.from(call.getOperands()).map(this::visit)), "type",
                        new TextNode(call.getType().toString())));
